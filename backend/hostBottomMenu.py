...
class ZoomBottomMenu:
    ...
    def get_curr_reaction_list(self, reaction, curr_list):
        if not self.participants_open:
            self.participants_open = self.click_bottom_menu("participants", self.participants_open)
        reaction_dict = {
        ...
        print(f"Reaction count for {reaction} is {reaction_count}")
    ...
    # After returning the participants name (if have), 
    # will pop the name from the reaction_list
    def get_next_person_with_reaction(self, reaction_list):
        if len(reaction_list) == 0:
            return None , reaction_list
        else:
            next_person_name = reaction_list.pop(0)
            return next_person_name, reaction_list

    def send_message_next_person(self, recipient_name):
        if recipient_name:
    ...
    # Below are all helper functions (UI wont use these functions below): 
    # Helper function for send_message_next_person
    ...
    # Helper functions for send_message
    ...
    # Helper functions for send_message, get_participants_list, 
    # get_curr_reaction_list, get_host_name,
